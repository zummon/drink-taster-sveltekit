...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-screen-md mx-auto">
	{#each data.blogs as item}
		<div
			class="bg-white bg-opacity-75 hover:bg-opacity-100 transition duration-500 rounded-lg"
		>
			<img class="rounded-t-lg aspect-video object-cover" src={item.cover.src} alt={item.cover.alt} />
